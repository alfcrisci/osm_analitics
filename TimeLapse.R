...
plot_single_timeunit <- function(before, this, total_by_timeunit, timeunit_pretty, basemap=basemap,titletxt="",base_color = 'grey50', highlight = 'green', bg_color = 'white', size = 1, alpha = 0.8) {
    this_timeunit <- unique(this$timeunit)
	year_lab=format(as.Date(floor_date(this_timeunit, "year")),format="%Y")
    ...
    p1 <- autoplot(basemap) + 
        ...
        labs(title = paste(timeunit_pretty,": ",year_lab,titletxt,sep = " ")) +  
...
time_lapse <- function(node_data_table, time_unit = 'year',basemap=basemap,
                       plot_single_timeunit_FUN = plot_single_timeunit, verbose = FALSE,titletxt="", ...) { 
    ...
    for (current_timeunit in sort(unique(node_data_table$timeunit))) {
        ...
        plot_single_timeunit_FUN(before = before, this = this,
                                 ...
                                 timeunit_pretty = R.utils::capitalize(time_unit),basemap=basemap,...)
        ani.pause()
    }
    if(verbose) cat('\n')
}
...
get_ggbasemap <- function(lat_range, lon_range, num_tiles = 9, type = "osm",
...
read_OSM <- function(osm_file, osmconvert = 'osmconvert', columns = '@lat @lon @timestamp @user @version') {
