...
matera_12_osh=read_OSM("matera_12_osh.osm")
basemap_type = "osm";
basemap_MT_12=get_ggbasemap(lat_range = range(matera_12_osh$lat, na.rm = T),lon_range = range(matera_12_osh$lon, na.rm = T), type = "osm", verbose = T,cache=F)
saveGIF({time_lapse(matera_12_osh, "year", basemap=basemap_MT_12,highlight="red",verbose = FALSE)}, movie.name = "matera_yearly.gif", interval = 2,extra.opts = "-dispose Background",clean = F, outdir = getwd())
gmap <- osm2ggmap(basemap_MT_12)

######################################################################################################################################à                               
# define structure extract data 

create_list_key(name="Matera",scale=12,file="Matera_12_osh.osm")
keys_MT=list_file_key_simple(name="Matera",scale=12,key="keys")	
keys_corpus_MT=generateCorpus_eng(paste(keys_MT$words,collapse=" "))
png("keys_corpus_MT_cloud.png")
wordcloud.osm(keys_corpus_MT,nsat=5,palette=rainbow(100))
dev.off()
key_tagmatrix_MT=DocumentTermMatrix(keys_corpus_MT)
key_zipf_MT=DTM_zipfs_model(key_tagmatrix_MT)                                
ggplotZipf(key_zipf_MT,"Matera","keys")
ggsave("Keys_Zipf_MT.png",width = 8,height = 8,dpi = 600) 

#######################################################à

key_sel=c("shop","amenity","tourism","man_made","natural","leisure","landuse","wikipedia")

res_tags_associated_MT=list()
res_mat_keys_MT=list()
res_mat_keys_uniqueid_MT=list()
res_mat_users_unique_MT=list()
res_mat_tags_corpus_MT=list()
key_tagmatrix_MT=list()
res_mat_tags_zipf_MT=list()

for ( i in 1:length(key_sel)) {
                                
								filter_file_key(name="Matera",scale=12,key=key_sel[i],file="Matera_12_osh.osm")
                                create_stat_keyvalue(name="Matera",scale=12,key=key_sel[i],file=paste0("Matera_12_",key_sel[i],".osm"))
                                res_tags_associated_MT[[i]]=list_file_key_simple(name="Matera",scale=12,key=key_sel[i])$words	
                                columns = paste('@lat @lon @timestamp @user @id @version',key_sel[i])
								res_mat_keys_MT[[i]]=read_OSM_full(paste0("Matera_12_",key_sel[i],".osm"),columns=columns)
				                res_mat_keys_MT[[i]]$key=key_sel[i]
								res_mat_users_unique_MT[[i]]=length(unique(res_mat_keys_MT[[i]]$user))
                                res_mat_keys_uniqueid_MT[[i]]=as.data.frame(res_mat_keys_MT[[i]][which(!duplicated(res_mat_keys_MT[[i]]$id))],)
								tags=as.character(as.data.frame(res_mat_keys_uniqueid_MT[[i]])[,7])
								tagsn=tags[which(!tags=="")]
			                    res_mat_tags_corpus_MT[[i]]=generateCorpus_eng(paste(tagsn,collapse=" "))
                                key_tagmatrix_MT[[i]]=DocumentTermMatrix(res_mat_tags_corpus_MT[[i]])
                                res_mat_tags_zipf_MT[[i]]=DTM_zipfs_model(key_tagmatrix_MT[[i]])                                
                                ggplotZipf(res_mat_tags_zipf_MT[[i]],"Matera",key_sel[i])
                                ggsave(paste0(key_sel[i],"_MT_Zipf.png"),width = 8,height = 8,dpi = 600) 
								names(res_mat_keys_uniqueid_MT[[i]])[7]="keyvalue"
                                png(paste0(key_sel[i],"_MT_wcloud.png"))
                                wordcloud.osm(res_mat_tags_corpus_MT[[i]],nsat=2,palette=rainbow(100))
                                dev.off()
								png(paste0(key_sel[i],"_MT_treemap_user_key.png"))
                                treemap(res_mat_keys_uniqueid_MT[[i]], index=c("key", "user"),vSize="version",type="index",title=paste("Matera",key_sel[i],"TreeMap Key - User"))
							    dev.off()
								png(paste0(key_sel[i],"_MT_treemap_key_value.png"))
                                treemap(res_mat_keys_uniqueid_MT[[i]][which(!tags==""),], index=c("key", "keyvalue"),vSize="version",type="index",title=paste("Matera",key_sel[i],"TreeMap Key with Value "))
							    dev.off()
...
res_mat_keyvalues_df=reshape::merge_all(res_mat_keys_uniqueid_MT)


# Create data.frame of user engaged

keyset_users_MT=data.frame(Nuser=c(unlist(res_mat_users_unique_MT)),Keyset=factor(c(key_sel)))


# Read all features of area

res_mat_keys_All=read_OSM_full("Matera_12_osh.osm")

# Create a vector and identify the length
res_mat_users_unique_All=length(unique(res_mat_keys_All$user))
ggplot(data=keyset_users_MT, aes(x=Keyset, y=Nuser))+ geom_bar(aes(fill=Keyset),stat="identity")+geom_text(aes(label=Nuser), vjust=3)+ ggtitle(paste("Matera Users Engaged in Keys (Ntot=",res_mat_users_unique_All,")"))
ggsave("Matera_12_users_engaged_keys.png", width = 8,height = 8,dpi = 600)                            

div_mod_MT <-diversity(res_mat_keyvalues_df$keyvalue, list(res_mat_keyvalues_df$key))
png("keys_diversity_MT_cloud.png",width = 1000, height = 800)
try(plot(div_mod_MT, high = "red", low = "yellow", grid = T, values = TRUE))
dev.off()

dis_mod_MT <-Dissimilarity(as.character(res_mat_keyvalues_df$keyvalue), list(as.character(res_mat_keyvalues_df$user)))
fit <- hclust(dis_mod_MT)
png("Cluster_user_MT_user_keyvalue.png")
plot(fit,main="Clustering Users by keyvalue wordset")
rect.hclust(fit, k=5, border=c("red", "purple", "seagreen","green","violet"))
dev.off()

dis_mod_MT <-Dissimilarity(as.character(res_mat_keyvalues_df$key), list(as.character(res_mat_keyvalues_df$user)))
fit <- hclust(dis_mod_MT)
png("Cluster_user_MT_user_key.png",width = 1000, height = 800)
plot(fit,main="Clustering Users by key wordset")
rect.hclust(fit, k=5, border=c("red", "purple", "seagreen","green","violet"))
dev.off()

dis_mod_MT <-Dissimilarity(as.character(res_mat_keyvalues_df$user), grouping.var =res_mat_keyvalues_df$year)
fit <- hclust(dis_mod_MT)
png("Cluster_user_MT_user_period.png",width = 1000, height = 800)
plot(fit,main="Clustering Year by User engaged")
rect.hclust(fit, k=2, border=c("red","seagreen"))
dev.off()

png("word_network_matera_keys_users.png",width = 1000, height = 800)
...
png("Venn_diagram_matera_keys_users.png",width = 1000, height = 800)
...
saveRDS(res_tags_associated_MT,"res_tags_associated_MT.rds")
saveRDS(res_mat_keys_MT,"res_mat_keys_MT.rds")
saveRDS(res_mat_keys_uniqueid_MT,"res_mat_keys_uniqueid_MT.rds")
saveRDS(res_mat_users_unique_MT,"res_mat_users_unique_MT.rds")
saveRDS(res_mat_users_unique_MT,"res_mat_users_unique_MT.rds")
saveRDS(res_mat_tags_corpus_MT,"res_mat_tags_corpus_MT.rds")
saveRDS(key_tagmatrix_MT,"key_tagmatrix_MT.rds")
saveRDS(res_mat_tags_zipf_MT,"res_mat_tags_zipf_MT.rds")
saveRDS(keyset_users_MT,"keyset_users_MT.rds")
saveRDS(res_mat_keyvalues_df,"res_mat_keyvalues_df_MT.rds")
###################################################################################################################################################àà
# Analisi per keys  analisi lessicale da fare il grafo di zipf

# Read raster 100 meter of city

r=raster("Matera_12.tif")
...
saveRDS(res_raster_full,"res_raster_full_MT.rds")
...
for ( i in as.numeric(range_year[1]+1):(as.numeric(range_year[2]))) {
                                         ...
                                         writeRaster(r_temp, paste0("matera_node_12_",i,".tif"),format="GTiff", overwrite=TRUE)
                                   	     writeRaster(r_frac, paste0("matera_FDvoid_12_",i,".tif"),format="GTiff", overwrite=TRUE)
					                     res_value_year_lac[[j]]=lacunarity(paste0("matera_node_12_",i,".tif"))
					                     res_value_year_fd_count[[j]]=frac.dim.box(paste0("matera_node_12_",i,".tif"))
                                         j=j+1					 
                                        }

										
png("r_fractal_dim_MT.png")
...
ggplot(time_lac, aes(Anno, Lac),color="red") + geom_line(color="red") + xlab("") + ylab("Lacunarity")+coord_cartesian(ylim=c(1,12))+ggtitle("Lacunarity  Year - MATERA S12 r 100m w 500m")
ggsave("Matera_12_year_lacunarity.png",width = 8,height = 8,dpi = 600) 
ggplot(time_fdc, aes(Anno, Fd)) + geom_line(color="red") + xlab("") + ylab("Fractal Dimension")+coord_cartesian(ylim=c(1,3))+ggtitle("Fractal Dimension  Year - MATERA S12 r 100m w 500m")
ggsave("Matera_12_year_fractal_dim.png",width = 8,height = 8,dpi = 600) 
ggplot(time_fea, aes(Anno, Feature)) + geom_line(color="red") + xlab("") + ylab("Number feature")+coord_cartesian(ylim=c(0,150000))+ggtitle("Volume features  Year - MATERA S12 r 100m ")
ggsave("Matera_12_year_volume_feature.png",width = 8,height = 8,dpi = 600) 

# Saving results

saveRDS(res_raster_year,"res_raster_year_MT.rds")
saveRDS(res_raster_year_frac,"res_raster_year_frac_MT.rds")
saveRDS(res_raster_year_fd,"res_raster_year_fd_MT.rds")
saveRDS(res_value_year_lac,"res_value_year_lac_MT.rds")
saveRDS(res_value_year_fd_count,"res_value_year_fd_count_MT.rds")
saveRDS(res_value_feature_count,"res_value_feature_count_MT.rds")
write.csv(time_lac,"year_lac_MT.csv",row.names=F)
write.csv(time_fdc,"year_fd_count_MT.csv",row.names=F)
write.csv(time_fea,"feature_count_MT.csv",row.names=F)
